...
def Parser.{u} (α : Type u) := String → Sum (String × α) String

def map_.{u}  {α β : Type u} (f : α → β) (a : Parser α) : Parser β :=
  fun (s : String) =>
    match a s with
    | inr e => inr e
    | inl (context, s') => inl (context, (f s'))
...
def pure_ (a : α) : (Parser α) := fun (s : String) => inl (s, a)

def seq_.{u} {α β : Type u} (f : Parser (α → β)) (a : Unit -> Parser α) : Parser β :=
  fun (s : String) => match f s with
    | inr e => inr e
    | inl (s', f') => match a () s' with
        | inr e => inr e
        | inl (s'', v) => inl (s'', (f' v))
...
def failure_.{u} : {α : Type u} → Parser α := inr
def orElse_.{u} {α : Type u} (p0 : Parser α) (p1 : Unit -> Parser α) : Parser α :=
  fun (str : String) => match p0 str with
    | inl r => inl r
    | inr e => match p1 () str with
      | inl r => inl r
      | inr e' => inr ("either " ++ e ++ " or " ++ e')
...
def charIf (p : Char -> Bool) : Parser Char :=
  fun (str : String) => match str.data with
    | [] => inr "empty string: no char match with predicate"
    | x :: xs => if p x
                  then inl ((String.mk xs), x)
                  else (inr <| x.toString ++ " doesn't match predicate")
